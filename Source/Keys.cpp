...
void Init::KeyPositions(std::map<std::string, ImVec2>& keyPositions) {
	...
	else if (*gLayoutIndex == 1) { // Full Keyboard
		keyPositions.clear(); // Clear existing positions

		if (*gLayoutIndex == 1 && *gUseMouseOverlay == true) {
			keyPositions[Mouse::Body] = ImVec2(0.5, 147.5);
...
void Init::KeyStates(std::map<std::string, KeyState>& keyStates, GameWrapper* gameWrapper)
{
	for (const auto& action : relevantActions) { // Iterate through relevantActions
		...
		keyStates[Key::PrintScreen] = { gameWrapper->GetFNameIndexByString("Pin"), false };
		...
		// ~ row
		keyStates[Key::Tilde] = { gameWrapper->GetFNameIndexByString("Tilde"), false };
		keyStates[Key::One] = { gameWrapper->GetFNameIndexByString("One"), false };
		keyStates[Key::Two] = { gameWrapper->GetFNameIndexByString("Two"), false };
		keyStates[Key::Three] = { gameWrapper->GetFNameIndexByString("Three"), false };
		keyStates[Key::Four] = { gameWrapper->GetFNameIndexByString("Four"), false };
		keyStates[Key::Five] = { gameWrapper->GetFNameIndexByString("Five"), false };
		keyStates[Key::Six] = { gameWrapper->GetFNameIndexByString("Six"), false };
		keyStates[Key::Seven] = { gameWrapper->GetFNameIndexByString("Seven"), false };
		keyStates[Key::Eight] = { gameWrapper->GetFNameIndexByString("Eight"), false };
		keyStates[Key::Nine] = { gameWrapper->GetFNameIndexByString("Nine"), false };
		keyStates[Key::Zero] = { gameWrapper->GetFNameIndexByString("Zero"), false };
		keyStates[Key::Minus] = { gameWrapper->GetFNameIndexByString("Underscore"), false };
		keyStates[Key::Equals] = { gameWrapper->GetFNameIndexByString("Equals"), false };
		...
		keyStates[Key::LBracket] = { gameWrapper->GetFNameIndexByString("LeftBracket"), false };
		keyStates[Key::RBracket] = { gameWrapper->GetFNameIndexByString("RightBracket"), false };
		keyStates[Key::Backslash] = { gameWrapper->GetFNameIndexByString("Backslash"), false };
		...
		keyStates[Key::Semicolon] = { gameWrapper->GetFNameIndexByString("Semicolon"), false };
		keyStates[Key::Quote] = { gameWrapper->GetFNameIndexByString("Quote"), false };
		...
		keyStates[Key::Comma] = { gameWrapper->GetFNameIndexByString("Comma"), false };
		keyStates[Key::Period] = { gameWrapper->GetFNameIndexByString("Period"), false };
		keyStates[Key::Forwardslash] = { gameWrapper->GetFNameIndexByString("Slash"), false };
...
void Assign::KeyboardActionRegions(
	...
	if (*gLayoutIndex == 1) { // Full Keyboard
